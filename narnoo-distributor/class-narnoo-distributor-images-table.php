	...
	function column_thumbnail_image( $item ) {    
		$actions = array(
			'add_to_album'  => sprintf( 
									...
									__( 'Add to album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ) 
								),
			'delete'    	=> sprintf( 
									'<a href="?%s">%s</a>', 
									build_query( 
		...
		if ( false !== $action ) {
			...
			switch ( $action ) {
				...
				case 'do_add_to_album':
					...
					<?php
					foreach( $image_ids as $id ) {
						Narnoo_Distributor_Helper::print_media_ajax_script_body( $id, 'addToAlbum', array( $id, $album_id ) );
					}
					?>
					</ol>
					<?php 
					Narnoo_Distributor_Helper::print_ajax_script_footer( $num_ids, __( 'Back to images', NARNOO_DISTRIBUTOR_I18N_DOMAIN ), __( 'View album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ) );
				...
				case 'delete':
					...
					<p><?php echo sprintf( __( 'Please confirm deletion of the following %d image(s):', NARNOO_DISTRIBUTOR_I18N_DOMAIN ), $num_ids ); ?></p>
					<ol>
					<?php 
					foreach ( $image_ids as $id ) { 
						?>
				...
				case 'do_delete':
					?>
					<h3><?php _e( 'Delete' ); ?></h3>
					<p><?php echo sprintf( __( "Deleting the following %s image(s):", NARNOO_DISTRIBUTOR_I18N_DOMAIN ), $num_ids ); ?></p>
					<ol>
					<?php
					foreach( $image_ids as $id ) {
