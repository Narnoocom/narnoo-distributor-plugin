	...
	function process_action() {
		...
		if ( false !== $action ) {
			...
			if ( $action === 'do_create' ) {
				...
				<ol>
				<?php
				Narnoo_Distributor_Helper::print_media_ajax_script_body( 'unknown', 'createAlbum', array( $new_album_name ) );
				?>
				</ol>
				<?php 
			...
			switch ( $action ) {
				...
				case 'remove':
					...
					?>
					<h3><?php _e( 'Confirm remove from album', NARNOO_DISTRIBUTOR_I18N_DOMAIN ); ?></h3>
					<p><?php echo sprintf( __( "Please confirm removal of the following %d image(s) from the album '%s' (ID %d):", NARNOO_DISTRIBUTOR_I18N_DOMAIN ), $num_ids, esc_html( $album_name ), $album_id ); ?></p>
				...
				case 'do_remove':
					...
					<?php
					foreach( $image_ids as $id ) {
						Narnoo_Distributor_Helper::print_media_ajax_script_body( $id, 'removeFromAlbum', array( $id, $album_id ) );
					}
					?>
					</ol>
					<?php 
